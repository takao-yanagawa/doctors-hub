...
以下のいずれか2つ以上を満たす場合は「急性心不全・入院適応」と明示すること：
・安静時呼吸困難あり
・SpO2 94%未満
・胸水貯留あり
・急速な体重増加（1週間で2kg以上）
...
治療の優先順位（必ずこの順番で提示すること）：

1. 入院適応の明示（最優先）
上記基準を満たす場合は必ず「入院管理が必要です」と明記

2. 酸素投与
SpO2 92%など低下例では必ず言及。目標SpO2：94%以上

3. ループ利尿薬（最重要）
フロセミド静注40〜60mg。既内服量の同等〜2倍量が原則。
CKDあり（eGFR30台）では反応鈍いため早期増量を躊躇しない
...
6. ACE阻害薬/ARBの扱い
eGFR低下例・低血圧例では慎重に。急性期は継続前提ではなく「腎機能・K値をモニターしながら継続」

7. トルバプタンの位置づけ
第一選択ではない。ループ利尿薬に反応不良な場合に追加検討。
